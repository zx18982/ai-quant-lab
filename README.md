# AI Quant Lab

股票技术指标交互式计算工具 — 纯前端实现，零构建依赖。

## 功能概览

- **6 只股票切换**：中芯国际（A/H 股）、比亚迪（A/H 股）、长江电力（A 股）、蓝思科技（A 股）
- **4 个技术指标实时计算**：RSI、MACD、布林带、ATR
- **参数滑块实时调节**：拖动释放后 150ms 防抖触发重绘
- **ECharts 图表渲染**：涨红跌绿配色，支持滚轮缩放、拖动平移、十字光标
- **前复权 K 线数据**：离线 JSON 数据源（多数标的约 250 个交易日；蓝思科技 `sz300433` 为最近 2 年约 485 个交易日）
- **补充诊断模块**：自动生成数据质量诊断、异常交易日、收益率分布、波动与序列特征、综合结论，随标的切换实时刷新

## 快速开始

```bash
# 方式一：直接用浏览器打开
open web_tool/index.html

# 方式二：启动本地服务器（推荐，避免 CORS 限制）
cd web_tool && python3 -m http.server 8765
# 浏览器访问 http://localhost:8765
```

## 项目结构

```
ai-quant-lab/
├── web_tool/                # 交互式指标计算工具
│   ├── index.html           # 主页面
│   ├── css/style.css        # 样式（涨红跌绿配色）
│   ├── js/
│   │   ├── indicators.js    # 指标计算引擎（纯手写 RSI/MACD/布林带/ATR）
│   │   ├── charts.js        # ECharts 多 grid 图表渲染
│   │   ├── diagnosis.js     # 补充诊断（数据质量 + 统计特征，纯前端）
│   │   └── app.js           # 主控制器（选股/参数/开关/导出/诊断）
│   └── data/
│       ├── stocks.json      # 股票配置
│       └── *_daily.json     # 6 只股票前复权日线数据（蓝思科技含 2 年）
├── spec.yaml                # 指标计算 notebook 规范文件
├── stock_data_spec.yaml     # 股票数据取数规范 (v1.2.0)
├── tool_design.md           # 产品设计文档
└── fetch_stock_data.py      # 数据获取脚本
```

## 指标说明

| 指标 | 核心回答的问题 | 类型 | 默认参数 |
|------|--------------|------|---------|
| RSI | 现在是不是涨/跌过头了？ | 震荡（动量） | 14 日，阈值 70/30 |
| MACD | 趋势方向有没有变？ | 趋势跟踪 | 12/26/9 日 |
| 布林带 | 价格偏离常态多远？何时变盘？ | 波动率通道 | 20 日，2 倍标准差 |
| ATR | 当前波动有多大？止损放哪？ | 波动率度量 | 14 日 |

## 技术栈

- HTML5 + CSS3 + 原生 JavaScript（零构建依赖）
- ECharts 5.4（CDN 加载）
- 数据源：westock-data（腾讯自选股）

## 补充诊断模块

页面底部「补充诊断 · 数据质量与统计特征」区块基于全部历史日线自动生成，切换标的实时刷新，包含 5 张卡片：

1. **数据质量诊断** — 健康度评分（重复日期 / OHLC 逻辑 / 零成交量 / 休市缺口校验）
2. **异常交易日** — 极端涨跌、跳空、放量（按幅度排序 Top 8）
3. **收益率分布统计** — 均值、年化波动率、偏度、超额峰度、极值、涨跌天数
4. **波动与序列特征** — 全样本 vs 近 20 日波动率、一阶自相关（动量/反转）
5. **综合诊断结论** — 趋势 / 分布 / 波动 / 序列整合判断与风险提示

## 部署

通过 GitHub Actions 自动发布：`push` 到 `main` 即触发工作流（`.github/workflows/pages.yml`），
将 `web_tool/` 目录作为静态站点部署到 GitHub Pages。

```bash
# 本地预览（避免 CORS）
cd web_tool && python3 -m http.server 8765
# 访问 http://localhost:8765
```

## License

MIT
